...
class RiseTogetherGame extends Forge2DGame
    with SingleGameInstance, HasKeyboardHandlerComponents, AppLogging {
  ...
  Future<void> onLoad() async {
    ...
    try {
      ...
      if (success) {
        appLog.fine('High refresh rate requested successfully.');
      } else {
        appLog.warning('Failed to enable high refresh rate');
      }
    } catch (e) {
      appLog.severe('Error: $e');
    }
    ...
    multicastLock
        ...
        .then((value) {
          appLog.fine('acquireMulticastLock: success');
        })
        .catchError((error) {
          appLog.severe('Failed to acquire multicast lock: $error');
        });
    ...
    appLog.fine('RiseTogetherGame loaded');
    ...
    await add(
      router = RouterComponent(
        routes: {
          ...
          'level': WorldRoute(LevelPage.new, maintainState: false),
  ...
  void onDispose() {
    ...
    multicastLock
        ...
        .then((value) {
          appLog.fine('releaseMulticastLock: success');
        })
        .catchError((error) {
          appLog.severe('Failed to release multicast lock: $error');
        });
    appLog.fine('RiseTogetherGame disposed');
    super.onDispose();
  }
...
class MenuPage extends Component
    with HasGameReference<RiseTogetherGame>, KeyboardHandler, AppLogging {
  String streamInfoText = 'Stream info';

  MenuPage() {
    addAll([
      appLogo = TextComponent(
        text: 'TITLE'.tr(),
      ...
      _button1 = RoundedButton(
        ...
        action: () {
          appLog.fine('MenuPage, level1 button pressed');
      ...
      _button2 = RoundedButton(
        ...
        action: () {
          appLog.fine('MenuPage, split screen button pressed');
      ...
      _button3 = RoundedButton(
        ...
        action: () {
          ...
          appLog.fine('Switched to ${game.teamId}');
        },
        color: const Color(0xffadde6c),
        borderColor: const Color(0xffedffab),
      ),
      _button4 = RoundedButton(
        ...
        action: () async {
          // log the action
          appLog.fine('MenuPage, find streams button pressed');
          // find streams
          ...
          if (streams.isNotEmpty) {
            ...
            appLog.fine('Found streams: ${streams.length}');
  ...
  late final TextComponent appLogo;
  ...
  void onGameResize(Vector2 size) {
    ...
    appLogo.position = Vector2(size.x / 2, size.y / 6);
    _participantId.position = Vector2(size.x / 2, appLogo.y + 60);
...
class RoundedButton extends PositionComponent with TapCallbacks, AppLogging {
  RoundedButton({
    required this.text,
  ...
  void onTapDown(TapDownEvent event) {
    ...
    appLog.fine('RoundedButton onTapDown');
  }

  @override
  void onTapUp(TapUpEvent event) {
    scale = Vector2.all(1.0);
    appLog.fine('RoundedButton onTapUp');
    action();
  }
  ...
  void onTapCancel(TapCancelEvent event) {
    scale = Vector2.all(1.0);
    appLog.fine('RoundedButton onTapCancel');
  }
}
...
class LevelPage extends DecoratedWorld
  ...
  void onLoad() async {
    ...
    paddlePos1 = Vector2(
      visibleRect.center.dx - 10 * appSettings['ball_radius'],
  ...
  @override
  void onRemove() {
    appLog.fine('LevelPage onRemove');
    super.onRemove();
  }
