...
class LevelResult {
  ...
  }) : winningTeam = team1Distance > team2Distance ? 0 : 
...
class RoundResult {
  ...
  int get roundWinner => team1Wins > team2Wins ? 0 : 
                        (team2Wins > team1Wins ? 1 : -1);
  
  bool get isComplete => levelResults.length >= 5; // Assuming 5 levels per round for now
}
...
class TournamentManager extends ChangeNotifier with AppLogging, Resetable {
  ...
  final Map<int, double> _currentLevelDistances = {0: 0.0, 1: 0.0}; // team -> max distance
  ...
  Map<int, double> get currentLevelDistances => Map.unmodifiable(_currentLevelDistances);
  ...
  String get levelProgress => 'Round ${_currentRound + 1}/$_totalRounds - Level ${_currentLevel + 1}/$_levelsPerRound';
  
  /// Tournament wins for each team
  int get team1RoundWins => _roundResults.where((r) => r.roundWinner == 0).length;
  int get team2RoundWins => _roundResults.where((r) => r.roundWinner == 1).length;
  ...
  void initialize(int rounds, int levelsPerRound) {
    ...
    appLog.info('Tournament initialized: $_totalRounds rounds x $_levelsPerRound levels');
    notifyListeners();
  }
  
  /// Update the maximum distance achieved by a team in the current level
  void updateTeamDistance(int teamId, double distance) {
    if (_currentLevelDistances[teamId] == null || distance > _currentLevelDistances[teamId]!) {
  ...
  void _completeRound() {
    ...
    appLog.info('Round ${_currentRound + 1} complete: Team 1: ${currentRoundResult.team1Wins} wins, Team 2: ${currentRoundResult.team2Wins} wins');
  ...
  void _completeTournament() {
    appLog.info('Tournament complete! Team 1 round wins: $team1RoundWins, Team 2 round wins: $team2RoundWins');
    final tournamentWinner = team1RoundWins > team2RoundWins ? 1 : 
